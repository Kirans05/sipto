...
import KeyIcon from '@mui/icons-material/Key';
import MailOutlineIcon from '@mui/icons-material/MailOutline';
import Styles from "../Styles/NewAssestPage.module.css"
import AttachMoneyIcon from '@mui/icons-material/AttachMoney';
import MessageIcon from '@mui/icons-material/Message';
import AddCircleOutlineIcon from '@mui/icons-material/AddCircleOutline';
import RemoveCircleOutlineIcon from '@mui/icons-material/RemoveCircleOutline';
  ...
  const fetchAllAssests = async () => {
    try{
      ...
      if(fetchResponse.status == 200){
        setAllAssests(fetchResponse.data)
      }
    }catch(err){
  ...
  return (
    <>
      ...
      <Box className={Styles.simpleMainBox}>
        ...
        <Box className={Styles.headerAndMainCompo}>
          <Header />
          <Box className={Styles.AssestsPageMainBox}>
            {
              allAssests.length == 0 ? <Typography>No Data Found</Typography>
              : <>
                {
                  allAssests.map((item, index) => {
                    return <CoinCard key={index} item={item} Styles={Styles}/>
                  })
                }
              </>
            }
