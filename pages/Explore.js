      ...
      if (resp.data) {
        if (basketRisk == '' && basketCost == 'All') {
          ...
          let cost =
            basketCost == 'All'
              ? 100000
              : basketCost == 'Less Than $50'
              ? 50
              : basketCost == 'Less Than $150'
              ? 150
              : 250;
          let filteredBasket = resp.data.filter(
            (item) =>
              item.risk.toLowerCase().startsWith(basketRisk.toLowerCase()) &&
              item.min_investment.amount < cost
          );
    ...
    try {
      ...
      if (resp.data) {
        let filteredData = resp.data.filter((item) =>
          item.name.toLowerCase().includes(inputFeildValue.toLowerCase())
        );
                      ...
                      style={{
                        backgroundColor:
                          basketRisk == 'Low'
                            ? 'rgb(255,215,111)'
                            : 'rgb(41,41,41)',
                        color: basketRisk == 'Low' ? 'black' : 'white'
                      }}
                      className={Styles.riskBtn}
                      onClick={() => setBasketRisk('Low')}
                    >
                      Low
                    </button>
                    <button
                      style={{
                        backgroundColor:
                          basketRisk == 'Med'
                            ? 'rgb(255,215,111)'
                            : 'rgb(41,41,41)',
                        color: basketRisk == 'Med' ? 'black' : 'white'
                      }}
                      className={Styles.riskBtn}
                      onClick={() => setBasketRisk('Med')}
                    >
                      Med
                    </button>
                    <button
                      style={{
                        backgroundColor:
                          basketRisk == 'High'
                            ? 'rgb(255,215,111)'
                            : 'rgb(41,41,41)',
                        color: basketRisk == 'High' ? 'black' : 'white'
                      }}
                      className={Styles.riskBtn}
                      onClick={() => setBasketRisk('High')}
                    >
                      High
                    </button>
                  </Box>
